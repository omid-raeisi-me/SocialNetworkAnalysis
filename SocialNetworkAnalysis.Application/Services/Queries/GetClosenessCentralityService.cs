...
{
    public class GetClosenessCentralityService : IGetClosenessCentralityService
    {
        private readonly IGraphRuntime _runtime;
        private readonly IClosenessCentrality _closenessCentralityAlgorithm;

        public GetClosenessCentralityService(IGraphRuntime runtime, IClosenessCentrality closenessCentralityAlgorithm)
        {
            _runtime = runtime;
        ...
        public ClosenessCentralityDto Execute()
        {
            ...
            var response = new ClosenessCentralityDto();
                ...
                if (coreResult.CentralityNodes != null)
                {
                    ...
                    {
                        response.CentralityNodes.Add(new User()
                        {
                ...
                if (coreResult.closenessScore != null)
                {
                    ...
                    foreach (var pair in sortedScores)
                    {
                        ...
                        response.ClosenessScores.Add(new User()
                        {
                            Name = userName,
                            Id = pair.Key
